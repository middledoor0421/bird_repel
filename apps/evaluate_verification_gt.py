...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate verification_result events against verification-camera GT.")
    parser.add_argument("--event-log", required=True, help="Path to a JSONL event log.")
    parser.add_argument("--frames", required=True, help="Frame annotation JSONL for verification camera(s).")
    parser.add_argument("--iou-threshold", type=float, default=0.1, help="Minimum IoU for GT-positive verification.")
    parser.add_argument(
        "--timestamp-tolerance-s",
        type=float,
        default=0.05,
        help="Max timestamp delta for pairing verification attempt with frame GT.",
    )
    ...
    summary = evaluate_verification_ground_truth(
    ...
    print(format_verification_gt_evaluation(summary))
    if args.output:
